...
void exit_with_help()
{
	...
	"	 7 -- DISDCA for L2-loss SVM\n"
	"	 8 -- DISDCA for L1-loss SVM\n"
	"-c cost : set the parameter C (default 1)\n"
	"-B bias : if bias >= 0, instance x becomes [x; bias]; if < 0, no bias term added (default -1)\n"
	"-f NFS_flag : if NFS_flag is true, then the data segments take the names in the form file_name.00, file_name.01,.... (all files take the same number of digits); if it is false, then each machine take the same file name (default 0)\n"
	...
	"  -s 0-5 :\n"
	"    primal and dual objective values are both within epsilon of opt\n"
	"  -s 6:\n"
	"    primal objective values is within epsilon of opt\n"
	);
	mpi_exit(1);
}
